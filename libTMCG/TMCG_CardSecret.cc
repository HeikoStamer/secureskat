...
TMCG_CardSecret::TMCG_CardSecret
	()
{
	r.push_back(vector<mpz_ptr>(1)), b.push_back(vector<mpz_ptr>(1));
	...
	(size_t n, size_t m)
{
	...
	for (size_t k = 0; k < n; k++)
		r.push_back(vector<mpz_ptr>(m)), b.push_back(vector<mpz_ptr>(m));
	...
	(const TMCG_CardSecret& that)
{
	for (size_t k = 0; k < that.r.size(); k++)
		r.push_back(vector<mpz_ptr>(that.r[k].size())),
		b.push_back(vector<mpz_ptr>(that.b[k].size()));
	for (size_t k = 0; k < r.size(); k++)
		for (size_t w = 0; w < r[k].size(); w++)
			mpz_init_set(r[k][w], that.r[k][w]), mpz_init_set(b[k][w], that.b[k][w]);
}
